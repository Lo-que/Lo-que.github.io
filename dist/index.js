;(function () {
...
})()(
  {
    1: [
      function (require, module, exports) {
        ...
        const lBirthday = new Date(2005, 1, 13, 23, 45)
        const anniversary = new Date(2025, 4, 4, 0, 10)
        const primaryEL = document.querySelector('.primary')
        const qEl = document.querySelector('.q')
        const lEl = document.querySelector('.f')
        const SECOND = 1000
        ...
        function render (el, now, milestone, percentage, isPrimary) {
          ...
          const totalYear =
            previousMilestone.getFullYear() - milestone.getFullYear()
          ...
          el.querySelector('.decimal').textContent = percentageYear
            .toFixed(3)
            .substr(1)
  ...
  `
        ...
        function update () {
          ...
          const lAge = now.getTime() - lBirthday.getTime()
          render(
            primaryEL,
            now,
            anniversary,
            (duration * 2) / (qAge + lAge),
            true
          )
          render(qEl, now, qBirthday, duration / qAge, false)
          render(lEl, now, lBirthday, duration / lAge, false)
